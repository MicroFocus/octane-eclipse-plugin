...
public class PluginPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private void saveValues() {
        try {
            ...
            logger.log(new Status(Status.ERROR, Activator.PLUGIN_ID, Status.ERROR,
                    "An exception has occured when saving the Core Software Delivery Platform connection details", e));
        }
    }

    private void setHints(boolean forServerUrlField) {
        if (forServerUrlField) {
            textServerUrl.setMessage("Copy paste your Core Software Delivery Platform URL from the browser here...");
        }
    ...
    private void testOctaneVersion(ConnectionSettings connectionSettings) {
        ...
        try {
            ...
            if (version.compareTo(OctaneVersion.DYNAMO) < 0) {
                new InfoPopup("Core Software Delivery Platform Settings",
                        "Core Software Delivery Platform version not supported. This plugin works with Core Software Delivery Platform versions starting " + OctaneVersion.DYNAMO.getVersionString(),
                        550, 100).open();
            }
            if (buttonBrowserAuth.getSelection() && version.compareTo(OctaneVersion.INTER_P2) < 0) {
                new InfoPopup("Core Software Delivery Platform Settings",
                        "Login with browser is only supported starting from Core Software Delivery Platform server version: " + OctaneVersion.INTER_P2.getVersionString(),
                        550, 100).open();
            ...
            message.append("Failed to determine Core Software Delivery Platform server version, http call to ")
            ...
            new InfoPopup("Core Software Delivery Platform Settings", message.toString(), 550, 100).open();
        }
    }
