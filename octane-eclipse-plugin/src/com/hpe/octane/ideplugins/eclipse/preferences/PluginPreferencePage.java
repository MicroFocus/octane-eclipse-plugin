...
public class PluginPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private static final String userPassAuthInfoText = "Log into OpenText™️ Core Software Delivery Platform directly with your user name and password, in non-SSO environments. " + System.lineSeparator() +
            "This method saves your login credentials between sessions, so you don’t have to re-enter them.";

    private static final String browserAuthInfoText = "Log into OpenText™️ Core Software Delivery Platform using a browser. " + System.lineSeparator() +
    ...
    private void loadSavedValues() {
        try {
            ...
            logger.log(new Status(Status.ERROR, Activator.PLUGIN_ID, Status.ERROR,
                    "An exception has occured when loading the OpenText™️ Core Software Delivery Platform connection details", e));
        }
    }
    ...
    private void saveValues() {
        try {
            ...
            logger.log(new Status(Status.ERROR, Activator.PLUGIN_ID, Status.ERROR,
                    "An exception has occured when saving the OpenText™️ Core Software Delivery Platform connection details", e));
        }
    }

    private void setHints(boolean forServerUrlField) {
        if (forServerUrlField) {
            textServerUrl.setMessage("Copy paste your OpenText™️ Core Software Delivery Platform URL from the browser here...");
        }
    ...
    private void testOctaneVersion(ConnectionSettings connectionSettings) {
        ...
        try {
            ...
            if (version.compareTo(OctaneVersion.DYNAMO) < 0) {
                new InfoPopup("OpenText™️ Core Software Delivery Platform Settings",
                        "OpenText™️ Core Software Delivery Platform version not supported. This plugin works with OpenText™️ Core Software Delivery Platform versions starting " + OctaneVersion.DYNAMO.getVersionString(),
                        550, 100).open();
            }
            if (buttonBrowserAuth.getSelection() && version.compareTo(OctaneVersion.INTER_P2) < 0) {
                new InfoPopup("OpenText™️ Core Software Delivery Platform Settings",
                        "Login with browser is only supported starting from OpenText™️ Core Software Delivery Platform server version: " + OctaneVersion.INTER_P2.getVersionString(),
                        550, 100).open();
            ...
            message.append("Failed to determine OpenText™️ Core Software Delivery Platform server version, http call to ")
            ...
            new InfoPopup("OpenText™️ Core Software Delivery Platform Settings", message.toString(), 550, 100).open();
        }
    }
